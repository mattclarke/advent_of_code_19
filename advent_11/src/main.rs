...
fn run_computer(commands: &mut Vec<i64>, inputs: Vec<i64>) -> Vec<i64> {
...
fn main() -> std::io::Result<()> {
    ...
    // Part 1 - we start on black
    // visited.entry(position).or_insert(0);
    // Part 2 - we start on white
    visited.entry(position).or_insert(1);

    while true {
        let pos = visited.entry(position).or_insert(0);
        let colour = *pos;

        let outputs = run_computer_index(&mut memory, vec![colour], index, relative_base);
        println!("outputs = {:?}", outputs);
        if memory[outputs.1 as usize] == 99 {
            // Part 1 = 2720
    ...
    for y in min_y..=max_y {
        let row: Vec<i32> = vec![0; (max_x - min_x + 1) as usize];
        board.push(row);
    }

    for (key, value) in &visited {
        if *value == 1 {
            board[(key.1 - min_y) as usize][(key.0 - min_x) as usize] = 1;
        }
    }
...
mod tests {
    ...
    #[test]
    fn it_works_for_day_11_example_1() {
        let original = vec![104, 1125899906842624, 99];
