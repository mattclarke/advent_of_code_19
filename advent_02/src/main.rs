...
pub fn calculate(mut vec: Vec<usize>) -> Vec<usize> {
    let mut index = 0;
    loop {
        if vec[index] == 1 {
...
pub fn string_to_ints(str: String, sep: char) -> Vec<usize> {
    let parts = str.split(sep);
    let vec = parts.flat_map(|x| x.parse::<usize>()).collect::<Vec<usize>>();
    return vec;
}
